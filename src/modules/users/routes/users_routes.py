...
async def get_user_by_id(user_id: int, _: dict = Depends(get_current_user)):
    return await users_controller.get_user_by_id(user_id)

@user_router.get("/get_by_email/{email}")
async def get_user_by_email(email: str, _: dict = Depends(get_current_user)):
    return await users_controller.get_user_by_email(email)

@user_router.get("/get_by_phone/{phone}")
async def get_user_by_phone(phone: str, _: dict = Depends(get_current_user)):
...
async def update_user(user_id: int, user: User, _: dict = Depends(get_current_user)):
